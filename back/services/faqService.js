...
class faqService {
    ...
    async faqAdd(title, q, a, token){
        ...
        const exit = await pgdb.query('INSERT INTO faq (useradd, usercheck, typeoffaq, dateadd, timeadd, qwest, ans, likes, dislikes, qwest_text_vector) values ($1, NULL, $2, current_date, localtime(0), $3, $4, 0, 0, to_tsvector($3)) RETURNING *', [decoded.data.id, title, q, a])
        return exit.rows[0]
    }
        ...
        if(titles.rowCount > 0){
            if(type == 'all' || type == 'part'){
                ...
                for (index = 0, len = titles.rowCount; index < len; ++index) {
                    let faqsByTitleArr
                    if(sort == 'down'){
                        faqsByTitleArr = await pgdb.query('SELECT faqid, typeoffaq, dateadd, timeadd, qwest, ans, likes, dislikes, users1.login AS useradd, users2.login AS usercheck FROM faq LEFT JOIN users AS users1 ON faq.useradd = users1.user_id LEFT JOIN users AS users2 ON faq.usercheck = users2.user_id WHERE typeoffaq = $1 ORDER BY faqid DESC LIMIT $2', [titles.rows[index].faqtypeid, count])
                    }
                    if(sort == 'top'){
                        faqsByTitleArr = await pgdb.query('SELECT faqid, typeoffaq, dateadd, timeadd, qwest, ans, likes, dislikes, users1.login AS useradd, users2.login AS usercheck FROM faq LEFT JOIN users AS users1 ON faq.useradd = users1.user_id LEFT JOIN users AS users2 ON faq.usercheck = users2.user_id WHERE typeoffaq = $1 ORDER BY (likes + dislikes) DESC, faqid DESC LIMIT $2', [titles.rows[index].faqtypeid, count])
                    }
    ...
    async searchFaq(str, limit){
        ...
        let take = await pgdb.query("SELECT faqid, typeoffaq, dateadd, timeadd, qwest, ans, likes, dislikes, users1.login AS useradd, users2.login AS usercheck, title FROM faq JOIN faqtype ON faqtype.faqtypeid = faq.typeoffaq LEFT JOIN users AS users1 ON faq.useradd = users1.user_id LEFT JOIN users AS users2 ON faq.usercheck = users2.user_id WHERE to_tsvector(lower(qwest)) @@ to_tsquery(lower($1)) ORDER BY ts_rank(to_tsvector(lower(qwest)), to_tsquery(lower($1))) DESC, (likes) DESC LIMIT $2", [newStr, limit])
        ...
        if(take.rowCount < limit){
            ...
            take = await pgdb.query("SELECT faqid, typeoffaq, dateadd, timeadd, qwest, ans, likes, dislikes, users1.login AS useradd, users2.login AS usercheck, title FROM faq JOIN faqtype ON faqtype.faqtypeid = faq.typeoffaq LEFT JOIN users AS users1 ON faq.useradd = users1.user_id LEFT JOIN users AS users2 ON faq.usercheck = users2.user_id WHERE faqid NOT IN ("+notForSearchId.join(',')+") AND lower(qwest) LIKE lower($1) ORDER BY (likes) DESC LIMIT $2", ['%'+str+'%', limit])
